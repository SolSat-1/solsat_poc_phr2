...
class SolarPredictionEngine:
    ...
    def optimize_panel_placement(
        self, roof_area: float, usable_factor: float = 0.75
    ) -> Dict:
        """
    ...
    def calculate_energy_production(
        self, panel_count: int, ghi: float, temperature: float = 25
    ) -> Dict:
        """
        Calculate energy production based on panels and conditions
        """
        # Temperature derating
        temp_factor = 1 + self.panel_specs["temperature_coefficient"] * (
            temperature - 25
        )

        # System efficiency including losses
        system_efficiency = (
            self.panel_specs["efficiency"]
            * (1 - self.panel_specs["system_losses"])
            * temp_factor
        )
        ...
        daily_energy_kwh = (
            ...
            * panel_area_total
            * system_efficiency
            * self.location_data["peak_sun_hours"]
        )
    ...
    def calculate_economic_analysis(
        ...
        installation_cost = (
            total_power_kw * 1000 * self.economic_params["installation_cost_per_watt"]
        )
        total_system_cost = equipment_cost + installation_cost

        # Annual savings
        annual_energy_value = (
            yearly_energy_kwh * self.economic_params["electricity_rate"]
        )
        annual_maintenance = (
            total_system_cost * self.economic_params["maintenance_annual"]
        )
        net_annual_savings = annual_energy_value - annual_maintenance

        # Monthly calculations
        monthly_energy_production = yearly_energy_kwh / 12
        monthly_energy_value = (
            monthly_energy_production * self.economic_params["electricity_rate"]
        )
        ...
        if monthly_energy_production > monthly_consumption_kwh:
            ...
            monthly_bill_reduction = (
                monthly_consumption_kwh * self.economic_params["electricity_rate"]
        ...
        # Payback period
        payback_years = (
            total_system_cost / net_annual_savings
    ...
    def get_color_palette_for_value(
    ...
    def analyze_rooftop(
        ...
        energy_data = self.calculate_energy_production(
            panel_data["panel_count"], irradiance_data["ghi"]
        )
        ...
        return {
            "roof_analysis": {"area_m2": roof_area, "coordinates": polygon_coords},
...
if __name__ == "__main__":
    ...
    print(
        f"Yearly Energy Production: {result['energy_production']['yearly_energy_kwh']:.2f} kWh"
    )
    print(
        f"Monthly Bill Reduction: ${result['economic_analysis']['monthly_bill_reduction']:.2f}"
    )
