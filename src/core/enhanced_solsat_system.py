...
from main.function.solar_prediction_engine import SolarPredictionEngine
...
class EnhancedSolarRooftopSystem:
    ...
    def analyze_all_rooftops(
        self,
        polygon_coords_list: List[List[Tuple[float, float]]],
        monthly_consumption_kwh: float = 500,
    ) -> List[Dict]:
        """
        ...
        for i, coords in enumerate(polygon_coords_list):
            analysis = self.prediction_engine.analyze_rooftop(
                coords, monthly_consumption_kwh
            )
    ...
    def create_enhanced_overlay_image(
        self, polygon, analysis_data, grid_shape, fname="enhanced_overlay.png"
    ):
        """
        ...
        rgb_colors = self.interpolate_colors(
            color_palette[0], color_palette[1], n_pix
        ).reshape(grid_shape[0], grid_shape[1], 3)
    ...
    def generate_overlay_grid_data(self, polygon, analysis_data, grid_size=50):
        """
        Generate overlay grid data suitable for frontend map rendering
        Returns numerical values that can be converted to colors/heatmap
        Fixed orientation for proper Google Maps overlay alignment
        """
        minx, miny, maxx, maxy = polygon.bounds

        # Create coordinate grids - IMPORTANT: ys should be from maxy to miny
        # for proper orientation with Google Maps
        xs = np.linspace(minx, maxx, grid_size)
        ys = np.linspace(maxy, miny, grid_size)  # Reversed for correct orientation
        xx, yy = np.meshgrid(xs, ys, indexing="xy")  # Use xy indexing

        # Create mask for polygon area
        points = np.column_stack((xx.ravel(), yy.ravel()))
        mask = np.array(
            [polygon.contains(Point(lon, lat)) for lon, lat in points]
        ).reshape(grid_size, grid_size)

        # Generate solar potential values based on analysis
        potential_score = analysis_data["solar_potential_score"]
        base_intensity = potential_score / 100.0

        # Create varied intensity pattern (simulating different roof areas)
        np.random.seed(42)  # For consistent results
        intensity_variation = np.random.normal(0, 0.1, (grid_size, grid_size))

        # Base intensity + small variations + distance-based falloff from center
        center_x, center_y = grid_size // 2, grid_size // 2
        intensity_grid = np.zeros((grid_size, grid_size))

        for i in range(grid_size):
            for j in range(grid_size):
                if mask[i, j]:  # Only inside polygon
                    # Distance from center (normalized)
                    dist = np.sqrt((i - center_x) ** 2 + (j - center_y) ** 2)
                    max_dist = np.sqrt(center_x**2 + center_y**2)

                    # Higher intensity at center, with variations
                    center_bonus = max(0, 1 - (dist / max_dist) * 0.3)
                    final_intensity = (
                        base_intensity * center_bonus + intensity_variation[i, j]
                    )
                    intensity_grid[i, j] = max(0, min(1, final_intensity))
                else:
                    intensity_grid[i, j] = 0  # Outside polygon

        return {
            "grid_size": grid_size,
            "bounds": {"north": maxy, "south": miny, "east": maxx, "west": minx},
            "values": intensity_grid.tolist(),
            "solar_score": potential_score,
            "color_mapping": {
                "low_threshold": 0.3,
                "medium_threshold": 0.6,
                "high_threshold": 0.8,
                "colors": {
                    "excellent": "#00FF00",  # Green > 80%
                    "good": "#FFFF00",  # Yellow 60-80%
                    "fair": "#FFA500",  # Orange 30-60%
                    "poor": "#FF6B6B",  # Light Red < 30%
                    "none": "#FFFFFF00",  # Transparent (outside)
                },
            },
        }
    ...
    def create_masked_overlay_image(
        self, polygon, grid_shape, palette, fname="masked_overlay.png"
    ):
        minx, miny, maxx, maxy = polygon.bounds
        ...
        mask = np.array(
            [polygon.contains(Point(lon, lat)) for lon, lat in lonlats]
        ).reshape(grid_shape)

        # Palette is (start, end), can be hex or RGB
        n_pix = grid_shape[0] * grid_shape[1]
        rgb_colors = self.interpolate_colors(palette[0], palette[1], n_pix).reshape(
    ...
    def render_enhanced_map(
        ...
        analysis_results = self.analyze_all_rooftops(
            polygon_coords_list, monthly_consumption_kwh
        )

        # Create summary statistics
        total_panels = sum(
            r["panel_optimization"]["panel_count"] for r in analysis_results
        )
        total_yearly_energy = sum(
            r["energy_production"]["yearly_energy_kwh"] for r in analysis_results
        )
        total_monthly_savings = sum(
            r["economic_analysis"]["monthly_bill_reduction"] for r in analysis_results
        )
        avg_solar_score = np.mean(
            [r["solar_potential_score"] for r in analysis_results]
        )
        ...
        for idx, (polygon, analysis) in enumerate(zip(polygons, analysis_results)):
            ...
            print(f"Creating overlay for polygon {idx + 1}...")

            if rgb:
                palette = list_palettes[idx % len(list_palettes)]
                sw, ne, fname = self.create_masked_overlay_image(
                    polygon, grid_shape, palette, fname
                )
            else:
                sw, ne, fname = self.create_enhanced_overlay_image(
                    polygon, analysis, grid_shape, fname
                )
    ...
    def generate_report(self) -> str:
        ...
        for i, analysis in self.analysis_results.items():
            ...
            report += "-" * 20 + "\n"
            report += (
                f"Solar Potential Score: {analysis['solar_potential_score']:.1f}/100\n"
            )
...
if __name__ == "__main__":
    ...
    coords1 = [
        ...
        (100.540577, 13.67167),
        (100.54055, 13.671899),
        (100.540534, 13.671972),
        (100.540239, 13.67193),
        (100.540255, 13.671847),
        (100.540148, 13.671842),
    ]
    coords2 = [
    ...
    coords4 = [
        (100.540325, 13.672155),
        (100.540363, 13.67193),
        (100.54054, 13.671967),
        (100.540577, 13.67191),
        (100.540727, 13.671962),
        (100.540684, 13.672181),
        (100.540325, 13.672155),
    ]
    coords5 = [
        (100.540593, 13.671503),
        (100.540668, 13.670982),
        (100.54092, 13.671018),
        (100.540904, 13.671159),
        (100.540963, 13.671169),
        (100.540878, 13.671534),
        (100.540593, 13.671503),
    ]
    coords6 = [
        (100.54062, 13.671889),
        (100.540647, 13.671649),
        (100.540776, 13.671665),
        (100.540733, 13.671915),
        (100.54062, 13.671889),
    ]
